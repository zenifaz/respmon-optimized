...
class RespiratoryMonitor:
    ...
    def initialize_pyqtgraph_visualization(self):
        # Create window using the modern QtWidgets module
        from pyqtgraph.Qt import QtWidgets
        app = QtWidgets.QApplication([])

        # Modern pyqtgraph replacement for the deprecated GraphicsWindow
    ...
    def update_ui(self):
            if self.visualize == 'pyqtgraph':
                if self.state == "calibration":
                    if self.calibration_buffer_idx < self.calibration_buffer_target_length:
                        ...
                        if self.current_frame is not None:
                            self.display_frame = cv2.rotate(self.current_frame, cv2.ROTATE_90_CLOCKWISE)
                    ...
                    # --- FIXED MEASUREMENT ROTATION HERE ---
                    raw_crop = float_to_uint8(self.cropped_image)
                    self.display_frame = cv2.rotate(raw_crop, cv2.ROTATE_90_CLOCKWISE)
                    
                    if self.motion_extraction_method == 'flow':
                        mask = np.zeros_like(self.display_frame)
                        # (Flow loops commented out to prevent crashing)
                ...
                # --- CLEANED UP EVENT PROCESSING AND EXIT HOOK ---
                from pyqtgraph.Qt import QtWidgets
                QtWidgets.QApplication.processEvents()
                
                # --- BULLETPROOF ESCAPE KEY EXIT HOOK ---
                import keyboard
                if keyboard.is_pressed('esc'):
                    print("\n[ESCAPE DETECTED] Cleaning up webcam and exiting cleanly...")
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            # --- NATIVE OPENCV KEYBOARD INTERCEPT ---
            # Listens for 1 millisecond on every single frame cycle
            key = cv2.waitKey(1) & 0xFF
            if key == 27 or key == ord('q'):  # 27 is the 'Esc' key, or you can press 'q'
                print("\n[EXIT DETECTED] Cleaning up webcam and exiting cleanly...")
                if hasattr(self, 'cap') and self.cap.isOpened():
                    self.cap.release()
                if hasattr(self, 'video_out') and self.video_out is not None:
            ...
            # Calibration phase
                ...
                if self.save_all_data and self.video_out is None:
                    self.video_out = cv2.VideoWriter(str(self.capture_target) + '.mp4',
                ...
                if self.save_all_data:
                    # 1. Convert the cropped image to standard uint8 format
                    gray_frame = float_to_uint8(self.cropped_image)
                    
                    # 2. Convert 1-channel gray to a 3-channel BGR image so Windows can play it
                    color_frame = cv2.cvtColor(gray_frame, cv2.COLOR_GRAY2BGR)
                    
                    # 3. Write the 3-channel frame to the file
                    self.video_out.write(color_frame)
